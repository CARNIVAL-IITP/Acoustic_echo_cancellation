...
from data.IITP_ES_finetune import IITP_ES3_dataloader
...
class trainer:
    def __init__(self, args, loss_type, resume):
        ...
        load_model_nm = self.model_name.rstrip('_UEHC')
        ...
        self.load_path =  args.output_path+'/%s_%s_%s_%s_%schan_%s'%(load_model_nm, self.dataset, self.loss_name, str(self.loss_type), str(args.feature_options.chan), args.feature_options.arr)
        self.min_loss = float("inf")
        self.early_stop_count = 0
        self.epoch = 0
        if self.resume == 'True':
            model_list = natsort.natsorted(glob.glob(self.load_path+'/*'))
            print(model_list[-1])
            self.model.load_state_dict(torch.load(model_list[-1], map_location='cpu'))
...
def main():
    ...
    parser.add_argument("-c", "--config", dest="path", default='./configs/train_UEHC.json',
    ...
    parser.add_argument("-r", "--resume", default='True')
