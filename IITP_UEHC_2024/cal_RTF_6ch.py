...
class tester:
    def __init__(self, args, loss_type):
        ...
        load_path_near_src = "./DB/"+mode1+"/mic/*_6ch_ellipsoid/*.wav"
    ...
    def test(self):
        ...
        with torch.no_grad():
            for idx, load_path_mic_nm in enumerate(tqdm(self.load_path_near_src_nm_list, desc=f'cal_RTF.py', dynamic_ncols=True,)):
                ...
                time_start = time()
                near_wav_hat = self.near_model([mic_wav[None, ...], far_wav[None, None, ...]])
                time_end = time()
...
def main():
    ...
    parser.add_argument("-c", "--config", dest="path", default='./configs/test_RTF_6ch.json',
